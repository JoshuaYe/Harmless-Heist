...
'''
Purpose:
- To update every object on the screen
Parameter:
update_List: The list of objects to update
Return:
- All objects are updated on the screen
'''
...
Purpose:
Parameter:
- To clear objects when needed
clear_List: The list of objects to clear
Return:
- All objects that are meant to be cleared are cleared
'''
...
Parameter:
draw_List: The list of objects to draw on the screen
Return:
- All objects are drawn onto the screen
'''
def draw(draw_List):
    for a in draw_List:
        a.draw(screen)
...
'''
Purpose:
- To kill sounds when needed
Parameter:
sounds_list: The list of sounds to kill
Return:
- Sounds are killed when needed
'''
...
def Level2(lightning_chance,lighting_time,lightning_flash,make_Walls,move_Char,key_Open,code_Open,doors,clicked,clicked_leave,distance,chase_move,money,random_money,money_score,win_money,gold,death,alarm,red_flash,screen_text,show_text,pause_game,police,drive_by,random_drive,key,drill,icon,chart,guard,ghost,sound_Zone,win_Zone,safe_Zone,leave,quit,board,board_flash):
    ...
    icon3 = icon("paper2.png",1240,-1950)
    ...
    road_clear = 0 ##Counter so that police do not overlap
    send_police = 0 ##Bool 1 send police 0 don't send
    distance_police = 0 ##Distance car has travelled

    police_group = pygame.sprite.Group()
    police1 = police()
    police_group.add(police1)

    update_List = [leave_group,board_group,board_flash_group,wall_group,key_group,drill,icon_group,safe_zone_group,square_group,szone_group,ghost_group,money_group,wzone_group,police_group]####################
    clear_List = [leave_group,board_group,board_flash_group,wall_group,key_group,drill,police_group,icon_group,safe_zone_group,guard_group,chart_group1,chart_group2,chart_group3,chart_group4,ghost_group,money_group,wzone_group]
    draw_List = [wall_group,guard_group,key_group,drill,safe_zone_group,money_group,ghost_group,police_group,square_group,icon_group,wzone_group]
    ...
    while keep_going:
        ...
        if road_clear == 0 and send_police < 2:
            send_police = random_drive()####################################################

        send_group = drive_by(police1,police_group,send_police,distance_police)
        send_police = send_group[0]
        police_group = send_group[1]
        distance_police = send_group[2]
        road_clear = distance_police
        ...
        x = move_X_Y_Dir[0]
        y = move_X_Y_Dir[1]
        #walkDir = move_X_Y_Dir[2]



        for i in money_group:
            money_gain = i.take_money(square_group,2)
        ...
        if pygame.sprite.groupcollide(square_group, police_group, False, False):
            kill_sounds(sounds_list)
            death(screen,money_total,minutes,seconds,0)
            keep_going = False
        if pygame.sprite.groupcollide(square_group, ghost_group, True, False):
            print("DEAD...")
            kill_sounds(sounds_list)
            death(screen,money_total,minutes,seconds,2)
            keep_going = False
